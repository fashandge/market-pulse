...
│   ├── market_overview.py    # Market overview assembler (SECTIONS, PORTFOLIO)
...
- Market Overview ticker groups come from `~/projects/stock_picker/data/ticker.csv` at runtime (`theme`, `display_order` columns; `exchange` must be the TradingView scanner value). Section ordering lives in `SECTIONS`, the hardcoded `Portfolio` list in `PORTFOLIO` (both `market_overview.py`). Full sourcing rules: [docs/architecture.md](docs/architecture.md#market-overview-data-sourcing-tickercsv--tradingview-scanner).
